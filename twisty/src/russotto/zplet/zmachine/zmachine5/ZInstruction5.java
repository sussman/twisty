...
public class ZInstruction5 extends ZInstruction
{
		...
		protected short z_call()
		{
				...
				if (operands[0] == 0) {
						...
						/* a non-storing call to zero is pretty ridiculous, but
						   hey, it could happen */
				}
				else {
...
//				System.err.println("aread " + Integer.toString(tbuf, 16) + " " + Integer.toString(operands[1]&0xFFFF,16));
...
//				zm.window[LOWER_WINDOW].setbuffermode(operands[0] != 0);
		...
		protected short op_tokenise()
		{
				...
				((ZDictionary5)zm.zd).tokenise(tbuf + 2, tlen, operands[1]&0xFFFF, parseunknown);
				return ZNOTDONE;
		}
