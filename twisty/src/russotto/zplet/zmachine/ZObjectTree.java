...
public abstract class ZObjectTree {
	...
	public short default_property(short property) {
				...
				property--;
				result = (short)(((zm.memory_image[object_table + property*2]<<8)&0xFF00) |
					(zm.memory_image[object_table + property*2 + 1] & 0xFF));
				return result;
	}
...
//				System.err.println(attr_num);
	...
	public int property_table_addr(short object) {
				...
				entryloc = getentryloc(object);
				return (((zm.memory_image[entryloc + ptableoffset()] << 8)&0xFF00) |
					(zm.memory_image[entryloc + ptableoffset() + 1] & 0xFF));
	}
...
//					   Integer.toString(default_property(propnum)&0xFFFF, 16));
					...
					if (size == 1)
								...
								return (short)
								(((zm.memory_image[entry_address+1]<<8) & 0xFF00) |
								(zm.memory_image[entry_address+2]&0xFF));
					}
				}
	}
